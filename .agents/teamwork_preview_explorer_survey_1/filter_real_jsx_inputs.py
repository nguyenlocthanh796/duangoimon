...
for root_dir in [os.path.join(ROOT, "app"), os.path.join(ROOT, "lib")]:
    for root, _, files in os.walk(root_dir):
        for f in files:
            ...
            # Match <TextInput with whitespace after (JSX element)
            # Exclude useRef<TextInput> or : TextInput
            for m in re.finditer(r'<TextInput\s', content):
                ...
                # Extract placeholder or purpose
...
with open(r"d:\duanpos-ongchu\.agents\teamwork_preview_explorer_survey_1\real_jsx_inputs.json", "w", encoding="utf-8") as fp:
    json.dump(jsx_inputs, fp, indent=2)
