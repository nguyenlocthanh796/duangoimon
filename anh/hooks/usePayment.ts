...
export const NUMPAD_KEYS: Array<{
  label: string;
  value: string;
  type: 'num' | 'clear' | 'back' | 'triple';
}> = [
...
export function usePayment({ tableId, tableName, total, orderId }: UsePaymentOptions) {
  ...
  useEffect(() => {
    if (orderId) {
      api
        .get<any>(`/ban-hang/orders/${orderId}`)
        .then((data) => {
          if (data && data.items) setOrderItems(data.items);
        })
        .catch((err) => logger.warn('payment', 'Could not load order items:', err));
    }
  }, [orderId]);
  ...
  const handlePay = async (paySplits?: { method: string; amount: number }[]) => {
    ...
    try {
      ...
      Alert.alert(
        'Lỗi thanh toán',
        e?.message || 'Không thể thực hiện thanh toán. Vui lòng thử lại.'
      );
    } finally {
      setPaying(false);
    }
  ...
  const handlePrint = () => {
    if (typeof window !== 'undefined') {
      ...
      if (!printWindow) {
        Alert.alert('Không thể in', 'Vui lòng cho phép popup để hiển thị hóa đơn in.');
        return;
      }
