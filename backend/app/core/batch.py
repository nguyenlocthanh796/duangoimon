...
async def batch_update(
    ...
    for i in range(0, len(ids), chunk_size):
        ...
        result = await db.execute(sa_update(model).where(model.id.in_(chunk_ids)).values(**values))
...
async def batch_delete(
    ...
    for i in range(0, len(ids), chunk_size):
        ...
        if soft and hasattr(model, "deleted_at"):
            ...
            result = await db.execute(sa_delete(model).where(model.id.in_(chunk_ids)))
        total += result.rowcount
    await db.commit()
    return total
