...
async def split_order(body: SplitOrderBody, db: AsyncSession = Depends(get_db), current_user: dict = Depends(get_current_user)):
    result = await db.execute(
        select(Order).options(selectinload(Order.items)).where(Order.id == uuid.UUID(body.order_id))
    )
    ...
    order.total_amount = sum(i.unit_price * i.quantity for i in keep_items)

    table_uuid = None
    if body.new_table_id:
        table_uuid = uuid.UUID(body.new_table_id)
...
async def move_table(order_id: str, body: MoveTableBody, db: AsyncSession = Depends(get_db), _user: dict = Depends(get_current_user)):
    ...
    new_table_uuid = uuid.UUID(body.table_id)
    existing = await db.execute(
    ...
    if target_order and str(target_order.id) != order_id:
        ...
        for item in order.items:
            ...
            db.add(item)
        target_order.total_amount = sum(i.unit_price * i.quantity for i in target_order.items)
        order.status = "da_gop"
        order.table_id = None
        if old_table_id:
...
async def merge_orders(body: MergeOrdersBody, db: AsyncSession = Depends(get_db), _user: dict = Depends(get_current_user)):
    result = await db.execute(
        select(Order).options(selectinload(Order.items)).where(Order.id == uuid.UUID(body.source_order_id))
    )
    ...
    if body.target_order_id:
        target_result = await db.execute(
            select(Order).options(selectinload(Order.items)).where(Order.id == uuid.UUID(body.target_order_id))
        )
    ...
    for item in source.items:
        ...
        db.add(item)
    target.total_amount = sum(i.unit_price * i.quantity for i in target.items)
...
async def cancel_order_item(body: CancelItemBody, db: AsyncSession = Depends(get_db), _user: dict = Depends(get_current_user)):
    result = await db.execute(select(OrderItem).where(OrderItem.id == uuid.UUID(body.item_id)))
