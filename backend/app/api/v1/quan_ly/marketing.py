...
def _campaign_dict(c: Campaign) -> dict:
    return {
        "id": str(c.id), "name": c.name, "type": c.type, "trigger": c.trigger,
        "segment_filters": c.segment_filters, "template": c.template,
        "scheduled_at": c.scheduled_at.isoformat() if c.scheduled_at else None,
        "sent_count": c.sent_count, "is_active": c.is_active,
...
async def create_campaign(body: CampaignCreate, db: AsyncSession = Depends(get_db), _user: dict = Depends(get_current_user)):
    scheduled = datetime.fromisoformat(body.scheduled_at) if body.scheduled_at else None
    c = Campaign(name=body.name, type=body.type, trigger=body.trigger,
                 segment_filters=body.segment_filters, template=body.template,
...
async def message_logs(db: AsyncSession = Depends(get_db), _user: dict = Depends(get_current_user)):
    ...
    return [
        {"id": str(l.id), "campaign_id": str(l.campaign_id), "customer_id": str(l.customer_id),
         "type": l.type, "status": l.status, "error": l.error,
