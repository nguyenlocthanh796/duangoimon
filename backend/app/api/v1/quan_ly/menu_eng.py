...
from app.models.ban_hang import OrderItem, Order, Product
...
async def boston_matrix(
    ...
    for p in products.scalars().all():
        ...
        product_map[str(p.id)] = {
            ...
            "cost_price": float(p.cost_price), "margin_pct": round(margin, 1),
...
async def top_bottom_sellers(
    ...
    rows = items.all()
    top = [{"name": r.product_name, "qty": r.total_qty or 0, "revenue": float(r.total_rev or 0)} for r in rows[:top_n]]
    bottom = [{"name": r.product_name, "qty": r.total_qty or 0, "revenue": float(r.total_rev or 0)} for r in rows[-top_n:]]
    bottom.reverse()
