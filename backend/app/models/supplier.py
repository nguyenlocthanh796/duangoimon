...
from sqlalchemy import DateTime, Date, ForeignKey, Integer, Numeric, String, Text, Boolean
...
class PurchaseOrder(Base):
    ...
    supplier_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("quan_ly.suppliers.id"))
    ...
    items: Mapped[list["PurchaseOrderItem"]] = relationship(backref="purchase_order", lazy="selectin", cascade="all, delete-orphan")
...
class PurchaseOrderItem(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    branch_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), nullable=True)
    po_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("quan_ly.purchase_orders.id", ondelete="CASCADE"))
