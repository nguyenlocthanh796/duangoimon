            ...
            {PAY_METHODS.map(m => {
              ...
              return (
                <TouchableOpacity key={m.id} onPress={() => pm.setMethod(m.id)}
                  ...
                  <Text style={{ ...font.bodySmall, fontWeight: '700', color: sel ? m.color : colors.text.muted, textAlign: 'center' }}>{m.label}</Text>
                  {sel && (
          ...
          {pm.method === 'tien_mat' && (
            <View style={{ backgroundColor: colors.surface.card, borderRadius: shape.radius.md, padding: 12, borderWidth: 1, borderColor: colors.border.default, gap: 10 }}>
              <View style={{ backgroundColor: colors.surface.app, borderRadius: shape.radius.md, paddingVertical: 12, alignItems: 'center', borderWidth: 1, borderColor: colors.border.default }}>
                <Text style={{ ...font.priceLarge, fontSize: 28, color: pm.cashInput ? colors.text.primary : colors.text.placeholder }}>
                  {pm.cashInput ? formatPriceFull(pm.cash) : '0đ'}
                </Text>
              </View>
              ...
              <View style={{ flexDirection: 'row', flexWrap: 'wrap', gap: 6 }}>
                {pm.smartSuggestions.slice(0, 6).map(amt => (
                  <TouchableOpacity key={amt} onPress={() => pm.setCashInput(String(amt))}
                    ...
                    <Text style={{ ...font.bodySmall, fontWeight: '700', color: pm.cash === amt ? colors.brand.primary : colors.text.primary }}>{formatPriceFull(amt)}</Text>
                  </TouchableOpacity>
                ))}
              </View>
            </View>
          ...
          {pm.method === 'chuyen_khoan' && (
            <View style={{ backgroundColor: colors.surface.card, borderRadius: shape.radius.md, padding: 16, borderWidth: 1, borderColor: colors.border.default, gap: 12, alignItems: 'center' }}>
              ...
              <View style={{ backgroundColor: '#FFF7ED', borderRadius: shape.radius.md, paddingVertical: 8, paddingHorizontal: 14, borderWidth: 1, borderColor: colors.border.brand, width: '100%', alignItems: 'center' }}>
                <Text style={{ ...font.bodySmall, fontWeight: '700', color: colors.brand.primary, textAlign: 'center' }}>TT {tableName} #{orderId?.slice(-6)}</Text>
              </View>
            </View>
          )}
        </ScrollView>
  ...
  return (
    <SafeAreaView edges={['left', 'right', 'bottom']} style={{ flex: 1, backgroundColor: colors.surface.app }}>
      <View style={{
        ...
        <TouchableOpacity onPress={() => router.back()} style={{ width: 42, height: 42, borderRadius: shape.radius.md, backgroundColor: colors.surface.disabled, alignItems: 'center', justifyContent: 'center' }}>
        ...
        <View style={{ flex: 1 }}>
          ...
          <Text style={{ ...font.caption, color: colors.text.muted, marginTop: 2 }}>Order #{orderId?.slice(-6) || '—'}</Text>
        </View>
        <View style={{ backgroundColor: colors.brand.primaryBg, paddingHorizontal: 14, height: 42, borderRadius: shape.radius.md, borderWidth: 1, borderColor: colors.border.brand, justifyContent: 'center', alignItems: 'center' }}>
