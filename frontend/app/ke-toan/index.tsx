...
// Fallback Dashboard Data Generator
    ...
    >
      ...
      <View style={styles.fbMetricContainer}>
        <View style={styles.fbMetricCard}>
          ...
          <View style={{ flex: 1 }}>
            ...
            {data.this_month?.thu_growth != null && (
              <AppText variant="sm" color="#65676B" style={{ fontSize: 11 }}>
                ↑ {data.this_month.thu_growth}% so với tháng trước
              </AppText>
            )}
          </View>
        </View>
        ...
        <View style={styles.fbMetricCard}>
          ...
          <View style={{ flex: 1 }}>
            ...
            {data.this_month?.chi_growth != null && (
              <AppText variant="sm" color="#65676B" style={{ fontSize: 11 }}>
                ↑ {data.this_month.chi_growth}% so với tháng trước
              </AppText>
            )}
          </View>
        </View>
        ...
        <View style={styles.fbMetricCard}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <AppText variant="sm" color="#65676B">Lợi nhuận</AppText>
            <AppText variant="sm" color="#65676B" style={{ fontSize: 11 }}>Biên lợi nhuận {profitPct}%</AppText>
          </View>
        </View>
        ...
        <View style={styles.fbMetricCard}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <AppText variant="sm" color="#65676B" style={{ fontSize: 11 }}>
              {s.exported_count ?? 99} đã xuất / {s.invoice_count ?? 100} tổng
            </AppText>
          </View>
        </View>
      </View>
      ...
      <View style={[styles.cardBox, { padding: 0 }]}>
        {(data.deadlines || []).map((d: any, i: number) => {
          ...
          const statusBg = urgent ? '#FEE2E2' : warn ? '#FEF3C7' : '#ECFDF5';
          const statusColor = urgent ? colors.status.danger : warn ? colors.status.warning : colors.status.success;

          return (
            <View key={i} style={[styles.deadlineRow, i === data.deadlines.length - 1 && { borderBottomWidth: 0 }]}>
              <View style={{ width: 8, height: 8, borderRadius: 4, backgroundColor: statusColor }} />
              <View style={{ flex: 1 }}>
                <AppText variant="md" weight="bold" color="#050505">{d.label}</AppText>
                <AppText variant="sm" color="#65676B">Hạn: {d.due}</AppText>
              </View>
              <View style={[styles.badgePill, { backgroundColor: statusBg, paddingHorizontal: 12, paddingVertical: 6 }]}>
                <AppText variant="sm" weight="bold" color={statusColor}>Còn {d.days_left} ngày</AppText>
              </View>
            </View>
          );
        })}
...
const styles = StyleSheet.create({
  ...
  /* Business Module Cards 4x2 Grid */
  moduleGrid: {
    flexDirection: 'row',
    flexWrap: 'wrap',
    gap: 10,
