  ...
  return (
    <View style={ss.sectionWrap}>
      ...
      <View style={s.tableHeader}>
        <AppText variant="sm" color="#64748B" style={{ flex: 1.5 }}>Ngày</AppText>
        <AppText variant="sm" color="#64748B" style={{ flex: 1, textAlign: 'center' }}>Số đơn / AOV</AppText>
        <AppText variant="sm" color="#64748B" style={{ flex: 1.5, textAlign: 'right' }}>Doanh thu</AppText>
      </View>
      {data.map((d, i) => {
          ...
          <View key={i} style={[s.tableRow, i % 2 === 0 && { backgroundColor: '#F8FAFC' }]}>
            <View style={{ flex: 1.5, flexDirection: 'row', alignItems: 'center', gap: 6 }}>
              <View style={{ width: 28, height: 20, borderRadius: 4, backgroundColor: '#F1F5F9', alignItems: 'center', justifyContent: 'center' }}>
                <AppText variant="sm" color="#475569" style={{ fontSize: 11 }}>{dayLabel || 'NG'}</AppText>
              </View>
              <AppText variant="sm" color="#0F172A">{formatDate(d.date)}</AppText>
            </View>
            <View style={{ flex: 1, alignItems: 'center' }}>
              <AppText variant="sm" color="#334155">{d.orders} đơn</AppText>
              {aov > 0 && (
                <AppText variant="sm" color="#64748B" style={{ fontSize: 11 }}>AOV {formatVND(aov)}</AppText>
              )}
            </View>
            <AppText variant="sm" weight="bold" color={colors.brand.primary} style={{ flex: 1.5, textAlign: 'right' }}>
  ...
  return (
    <View style={ss.sectionWrap}>
      ...
      {data.length === 0 ? (
        ...
        <>
          <View style={s.tableHeader}>
            <AppText variant="sm" color="#64748B" style={{ width: 36 }}>Hạng</AppText>
            <AppText variant="sm" color="#64748B" style={{ flex: 1 }}>Sản phẩm</AppText>
            <AppText variant="sm" color="#64748B" style={{ width: 65, textAlign: 'center' }}>Đã bán</AppText>
            <AppText variant="sm" color="#64748B" style={{ width: 95, textAlign: 'right' }}>Doanh thu</AppText>
          </View>
          {data.map((p, i) => {
              ...
              <View key={i} style={[s.tableRow, i % 2 === 0 && { backgroundColor: '#F8FAFC' }]}>
                <View style={{ width: 36, alignItems: 'center' }}>
                  <View style={{ width: 22, height: 20, borderRadius: 4, backgroundColor: rankBg, alignItems: 'center', justifyContent: 'center' }}>
                    <AppText variant="sm" weight="bold" color={rankColor} style={{ fontSize: 11 }}>#{i + 1}</AppText>
                  </View>
                </View>
                <AppText variant="sm" color="#0F172A" style={{ flex: 1, paddingLeft: 4 }} numberOfLines={1}>
                  {p.name}
                </AppText>
                <View style={{ width: 65, alignItems: 'center' }}>
                  <View style={{ backgroundColor: '#F8FAFC', borderWidth: 1, borderColor: '#E2E8F0', borderRadius: 4, paddingHorizontal: 6, paddingVertical: 1 }}>
                    <AppText variant="sm" color="#334155" style={{ fontSize: 11 }}>{p.quantity}</AppText>
                  </View>
                </View>
