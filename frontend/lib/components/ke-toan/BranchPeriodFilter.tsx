...
export default function BranchPeriodFilter({
  ...
  useEffect(() => {
    ...
    api.getBranches().then((b) => { if (active) setBranches(b); }).catch(() => {}).finally(() => { if (active) setLoadingBranches(false); });
    return () => { active = false; };
  }, []);
    ...
    <View style={styles.bar}>
      ...
      >
        {loadingBranches ? (
          ...
          <Icon name={locked ? 'lock-outline' : 'store-outline'} size={18} color={colors.icon.muted} />
        )}
        ...
        <View style={styles.formRow}>
          {formOptions.map((o) => (
            <TouchableOpacity
              ...
              activeOpacity={0.7}
            >
              <Text style={[styles.formChipText, form === o.key && styles.formChipTextActive]} numberOfLines={1}>
      ...
      {!locked && (
        <FormModal visible={pickerOpen} title="Chọn chi nhánh" onClose={() => setPickerOpen(false)}>
          {loadingBranches ? (
            <View style={styles.centerBox}><ActivityIndicator size="large" color={colors.brand.primary} /></View>
          ) : branches.length === 0 ? (
            <View style={styles.centerBox}>
              <Icon name="store-outline" size={48} color={colors.text.muted} />
              <Text style={[font.body, { color: colors.text.muted, marginTop: 8, textAlign: 'center' }]}>Không có chi nhánh nào</Text>
            </View>
          ) : (
            branches.map((b) => (
              <TouchableOpacity
                ...
                onPress={() => { onBranchChange(b.id); setPickerOpen(false); }}
                activeOpacity={0.7}
              >
                <View style={styles.branchItemIcon}>
                  <Icon name="store-outline" size={20} color={b.id === branchId ? colors.text.inverse : colors.brand.primary} />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={[styles.branchItemName, b.id === branchId && styles.branchItemNameActive]}>{b.name}</Text>
                  {b.tax_code ? <Text style={[font.caption, { color: colors.text.muted }]}>MST: {b.tax_code}</Text> : null}
                </View>
...
const styles = StyleSheet.create({
  bar: { flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: 16, paddingVertical: 10, backgroundColor: colors.surface.card, borderBottomWidth: 1, borderBottomColor: colors.border.light, flexWrap: 'wrap' },
  branchBtn: { flex: 1, minWidth: 140, flexDirection: 'row', alignItems: 'center', gap: 8, backgroundColor: colors.surface.app, borderRadius: shape.radius.md, borderWidth: 1, borderColor: colors.border.default, paddingHorizontal: 12, height: 44 },
  ...
  inputWrap: { backgroundColor: colors.surface.app, borderRadius: shape.radius.md, borderWidth: 1, borderColor: colors.border.default, paddingHorizontal: 12 },
  input: { height: 44, ...font.body, color: colors.text.primary },
  formRow: { flexDirection: 'row', gap: 6, width: '100%', marginTop: 4 },
  formChip: { flex: 1, paddingVertical: 8, paddingHorizontal: 10, borderRadius: shape.radius.md, backgroundColor: colors.surface.app, borderWidth: 1, borderColor: colors.border.default, alignItems: 'center' },
  ...
  branchItem: { flexDirection: 'row', alignItems: 'center', gap: 12, padding: 14, borderRadius: shape.radius.md, borderWidth: 1, borderColor: colors.border.light, marginBottom: 8, backgroundColor: colors.surface.card },
  branchItemActive: { borderColor: colors.border.brand, backgroundColor: colors.brand.primaryBg },
  branchItemIcon: { width: 38, height: 38, borderRadius: shape.radius.md, backgroundColor: colors.brand.primaryBg, alignItems: 'center', justifyContent: 'center' },
