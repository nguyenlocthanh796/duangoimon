...
export default function InvoiceFormContent({ form, setForm, errors, setErrors, paidOrders, ordersLoading }: Props) {
  ...
  return (
    <>
      ...
      <TouchableOpacity style={[styles.input, styles.pickerRow, errors.order_id && styles.inputError]}
        onPress={() => setShowPicker(true)} activeOpacity={0.7}>
        {ordersLoading ? (
          <ActivityIndicator size="small" color={colors.brand.primary} />
        ) : (
          <>
            <Text style={{ flex: 1, fontSize: 15, color: form.order_id ? colors.text.primary : colors.text.placeholder }} numberOfLines={1}>
              {selectedOrder ? `${selectedOrder.table_name} · #${selectedOrder.id.slice(-6).toUpperCase()}` : 'Chọn đơn hàng đã thanh toán'}
            </Text>
        ...
        <View style={styles.sheet}>
          ...
          {paidOrders.length === 0 ? (
            <View style={{ alignItems: 'center', paddingVertical: 24, gap: 8 }}>
              ...
              <Text style={{ color: colors.text.muted, fontSize: 13 }}>Không có đơn hàng đã thanh toán</Text>
            </View>
          ) : (
            <ScrollView style={{ flex: 1 }} keyboardShouldPersistTaps="handled">
              {paidOrders.map(o => (
                <TouchableOpacity key={o.id} style={styles.orderRow}
                  onPress={() => { setForm(f => ({ ...f, order_id: o.id })); setErrors(e => ({ ...e, order_id: undefined })); setShowPicker(false); }}>
                  <View style={styles.orderDot} />
                  <View style={{ flex: 1 }}>
                    <Text style={styles.orderName}>{o.table_name || `Đơn #${o.id.slice(-6)}`}</Text>
                    {o.total != null && <Text style={styles.orderTotal}>{o.total.toLocaleString('vi-VN')}₫</Text>}
                  </View>
      ...
      <TextInput style={[styles.input, errors.buyer_name && styles.inputError]}
        placeholder="Nhập tên người mua" placeholderTextColor={colors.text.placeholder}
      ...
      <TextInput style={styles.input} placeholder="Mã số thuế (không bắt buộc)" placeholderTextColor={colors.text.placeholder}
        value={form.buyer_tax_code} onChangeText={(v) => setForm(f => ({ ...f, buyer_tax_code: v }))} />

      <Text style={styles.label}>Thuế suất VAT (%)</Text>
      <TextInput style={[styles.input, errors.vat_rate && styles.inputError]}
        placeholder="10" placeholderTextColor={colors.text.placeholder}
        keyboardType="numeric" value={form.vat_rate}
...
const styles = StyleSheet.create({
  ...
  sheet: {
    backgroundColor: colors.surface.card, borderTopLeftRadius: 20, borderTopRightRadius: 20,
    padding: 24, paddingBottom: 40,
  },
  handle: { width: 36, height: 4, borderRadius: 2, backgroundColor: colors.border.default, alignSelf: 'center', marginBottom: 20 },
  sheetTitle: { ...font.h3, color: colors.text.primary, marginBottom: 16 },
  orderRow: { flexDirection: 'row', alignItems: 'center', paddingVertical: 12, paddingHorizontal: 4, borderBottomWidth: StyleSheet.hairlineWidth, borderBottomColor: colors.border.light },
  orderDot: { width: 8, height: 8, borderRadius: 4, backgroundColor: colors.brand.primary, marginRight: 12 },
