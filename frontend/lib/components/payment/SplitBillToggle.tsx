...
export default function SplitBillToggle({
  showSplitter, setShowSplitter, splits, setSplits, handlePay, paying, canPay, total,
}: Props) {
  return showSplitter ? (
    <SplitBillPanel
      total={total}
      splits={splits}
      onChange={setSplits}
      onPay={() => { setShowSplitter(false); handlePay(splits.length > 0 ? splits : undefined); }}
      onCancel={() => setShowSplitter(false)}
      paying={paying}
      canPay={canPay}
    />
  ) : (
    <TouchableOpacity onPress={() => setShowSplitter(true)}
      style={{ alignItems: 'center', padding: 6, marginTop: 4, flexDirection: 'row', justifyContent: 'center', gap: 4 }}>
