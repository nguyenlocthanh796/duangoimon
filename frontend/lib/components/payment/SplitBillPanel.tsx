...
export default function SplitBillPanel({ total, splits, onChange, onPay, onCancel, paying, canPay }: Props) {
  ...
  const updateSplit = (idx: number, field: 'method' | 'amount', value: string | number) => {
    onChange(splits.map((s, i) => i === idx ? { ...s, [field]: typeof value === 'string' ? value : Math.max(0, value) } : s));
  };
  ...
  return (
    <View style={{ padding: 12, backgroundColor: colors.surface.card, borderTopWidth: 1, borderTopColor: colors.border.default, gap: 8 }}>
      ...
      {splits.map((sp, idx) => (
        <View key={idx} style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
          ...
          <TextInput
            value={String(sp.amount)} onChangeText={(v) => updateSplit(idx, 'amount', parseFloat(v) || 0)}
            keyboardType="decimal-pad"
            style={{ borderWidth: 1, borderColor: colors.border.default, borderRadius: 8, padding: 6, ...font.caption, color: colors.text.primary, width: 90, textAlign: 'center' }}
          />
          <TouchableOpacity onPress={() => removeSplit(idx)}><Icon name="close-circle" size={18} color={colors.status.danger} /></TouchableOpacity>
        </View>
      ))}
      <TouchableOpacity onPress={addSplit} style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
      ...
      <TouchableOpacity onPress={onPay} disabled={!ok || paying}
        style={{ paddingVertical: 12, borderRadius: 10, alignItems: 'center', backgroundColor: ok && !paying ? colors.status.success : colors.surface.disabled }}>
        <Text style={{ ...font.h3, color: !ok || paying ? colors.text.muted : colors.text.inverse }}>
