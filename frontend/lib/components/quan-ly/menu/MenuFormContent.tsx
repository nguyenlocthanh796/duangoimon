...
interface MenuFormContentProps {
  form: { code: string; name: string; category: string; price: string; cost_price: string; unit: string; is_active: boolean };
        ...
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 8, paddingBottom: 4 }}>
          {CATEGORIES.map(cat => {
            ...
            return (
              <TouchableOpacity
                key={cat}
                style={[styles.catChip, sel && { backgroundColor: colors.brand.primary, borderColor: colors.brand.primary }]}
                onPress={() => onChange({ category: form.category === cat ? '' : cat })}
              >
                <Text style={[styles.catChipText, sel && { color: colors.text.inverse }]}>{cat}</Text>
              </TouchableOpacity>
            );
          })}
        </ScrollView>
...
const styles = StyleSheet.create({
  ...
  switchRow: {
    ...
    paddingVertical: 14, borderTopWidth: 1, borderTopColor: colors.border.light, marginTop: 4,
  },
